...
3. 데모 API는 `/api/**`가 아니라 `/demo-api/**`에 격리해 CT-05의 실제 `/api/**`와 구분한다.
...
- [x] `origin/main`의 PR #8 결과를 현재 STEP 06 브랜치에 반영한다.
- [x] Flask 서버를 제거하고 같은 화면 기능을 제공하는 Java Spring 데모 컨트롤러·상태 저장소를 추가한다.
- [x] 웹 정적 파일을 Spring Boot 실행 JAR에 포함하고 API prefix를 `/demo-api`로 변경한다.
- [x] 입력 검증, 없는 자원, 잘못된 분류·보관 기간, 삭제 상태의 계약 테스트를 추가한다.
- [x] 기본 프로필에서 데모 API가 노출되지 않고 `demo` 프로필에서만 동작하는지 검증한다.
- [x] 기존 `verifyAll`, Android clean build·lint, 서버 실행 JAR 검증을 다시 실행한다.
- [x] STEP 06 보고서·상태·audit에 PR #8 보정 결과와 남은 실기기 경계를 기록한다.
