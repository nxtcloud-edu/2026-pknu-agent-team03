...
3. 대상이 보호 파일이면 오류 메시지와 exit 2를 반환해 도구 실행을 차단한다.
4. 보호 대상이 아니면 exit 0으로 통과시키며, append 전용 fs_append는 hook matcher 대상이 아니므로 허용된다.
...
foreach ($pathValue in $pathValues) {
    ...
    foreach ($protected in $protectedSuffixes) {
        if ($normalized -eq $protected -or $normalized.EndsWith('/' + $protected)) {
            [Console]::Error.WriteLine("Blocked destructive write to append-only record: $pathValue. Use fs_append instead.")
            exit 2
        }
    }
}
